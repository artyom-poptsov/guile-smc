...
$ cd guile-smc
$ autoreconf -vif
$ ./configure --with-guilesitedir=<path-to-guile-site> --prefix=<prefix>
$ make
$ sudo make install
#+END_EXAMPLE

Where =<path-to-guile-site>= is the path where Guile modules are stored
(usually something like =/usr/share/guile/site/3.0=) and =<prefix>= is the
...
Usage:
#+BEGIN_EXAMPLE
smc context [options] [input-file]
#+END_EXAMPLE
...
***** =--resolve=, =-r=
Show resolved and unresolved procedures.
...
- functional/u8
***** =--generate=, =-g=
Generate a context stub from a given PlantUML file.
***** =--guile-smc-path= =<path>=
Set the path where Guile-SMC modules are stored.

Default value is set to the current Guile-SMC modules installation path.
***** =--module=, =-m= <module>
...
- "file" -- log to a specified file. Output files are rotated as
  needed. Options: "file" -- the full path to the log file.
...
Example: "file=/tmp/smc.log"

There's an option "stderr" that is handled independently from the log driver
that allows to configure stderr logging.  Example values:
- "stderr=true"
- "stderr=false"
...
Usage:
#+BEGIN_EXAMPLE
smc compile [options] [input-file]
#+END_EXAMPLE
...
Print the FSM transition table to the standard output.
...
Supported values:
- "syslog" -- use syslog as the logging driver.
- "file" -- log to a specified file. Output files are rotated as needed.
  Options: "file" -- the full path to the log file.
- "null" -- disable logging (discard all the messages.)
...
Load additional modules.  The value must be the same as for =use-modules=.

Example value: "((smc context char-context) (smc puml-context))"

***** =--fsm-name=, =-n= =<name>=
Set the name for the output FSM.

***** =--fsm-module=, =-m= =<module>=
Set the module for the output FSM.

Example value: "(smc puml-fsm)"

***** =--validate=
Validate the output FSM and print the validation result.  The exit code is 0
if the validation is passed, and a non-zero value otherwise.

***** =--target=, =-t= =<target>=
Compilation target.  Allowed values: "guile", "guile-standalone"
...
Example value: "(lambda (context) (display context))"
***** =--load-path=, =-L= =<load-path>=
Add an extra load path.
...
There is an option "stderr" that is handled independently from the log driver
that allows to configure stderr logging.
